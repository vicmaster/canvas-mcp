...
- **`canvas_evaluate`** scores the design on 7 categories (spacing, color, typography, structure, consistency, cliche, coverage) and surfaces actionable issues with `nodeId` references. The `coverage` category demands designed state variants on data-bearing screens: a table without an `empty` + `loading` variant (or a form without an `error` variant) is a directive-blocking warning — `canvas_add_variant` + the `empty-state`/`skeleton-table` scaffolds are the one-call path to comply; the result's `coverage` field reports `{ dataBearing, states, missing }`. Use it in a generator-evaluator loop: `batch_design` → `canvas_evaluate` → fix the returned nodeIds.
